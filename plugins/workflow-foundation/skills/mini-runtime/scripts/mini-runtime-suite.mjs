...
const scriptName = 'mini-runtime-suite.mjs';
const candidates = [
...
child.on('error', (error) => {
  console.error(`Failed to start canonical mini-runtime suite script: ${error.message}`);
  process.exit(1);
});

child.on('exit', (code, signal) => {
  if (signal) {
    console.error(`Canonical mini-runtime suite script exited with signal ${signal}`);
    process.exit(1);
  }
  process.exit(code ?? 1);
});
