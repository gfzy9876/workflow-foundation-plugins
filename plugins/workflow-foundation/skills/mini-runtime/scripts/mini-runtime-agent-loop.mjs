...
const scriptName = 'mini-runtime-agent-loop.mjs';
const candidates = [
...
child.on('error', (error) => {
  console.error(`Failed to start canonical mini-runtime agent-loop script: ${error.message}`);
  process.exit(1);
});

child.on('exit', (code, signal) => {
  if (signal) {
    console.error(`Canonical mini-runtime agent-loop script exited with signal ${signal}`);
    process.exit(1);
  }
  process.exit(code ?? 1);
});
